...
这一目录是 Android 端的内部扩展架构，不是动态下载、安装第三方 APK 的插件市场。它的目标是让后续新增或大幅改造功能先依赖稳定 API，而不是直接耦合现有页面、Repository 或持久化细节。本次只建立旁路能力；现有 Screen、ViewModel、导航和数据调用链没有迁移。
...
未来插件模块
  └─ Plugin.onLoad(PluginContext)
       ...
       ├─ AIAPI    ── AiApiAdapter    ── AiChatRepository
...
- `plugin-api/core`：独立 Android library，保存 `Plugin`、`PluginContext`、`PluginManager`、七类 API 契约、跨边界 DTO 和 Compose UI contribution 类型。
...
3. 日记、Markdown、媒体、同步和 AI 操作分别通过 `context.diary`、`vault`、`media`、`sync`、`ai` 发起。不要直接注入 app 内 Repository，也不要自行解析 SAF URI、云凭据或 AI Key。
4. 插件状态只通过 `context.storage` 保存。该命名空间按插件 ID 隔离，不增加 Room 表、不属于 `AppSettings`，也不会改变 DeskCubby v28 JSON。
...
需要交付 APK 时，继续执行仓库根文档中的 Android JVM 测试、Android test 源码编译、`assembleDebug` 与 Lint。插件架构不改变 Room v12、DeskCubby JSON v28、Markdown 或 `dc-media.json` v2。
