...
enum ManifestBodyError {
    Empty,
    TooLarge,
}

fn validate_declared_manifest_length(
    content_length: Option<u64>,
) -> std::result::Result<(), ManifestBodyError> {
    match content_length {
        Some(0) => Err(ManifestBodyError::Empty),
        Some(length) if length > MAX_RELEASE_MANIFEST_BYTES as u64 => {
            Err(ManifestBodyError::TooLarge)
        }
        _ => Ok(()),
    }
}

fn checked_manifest_length(
    current_length: usize,
    chunk_length: usize,
) -> std::result::Result<usize, ManifestBodyError> {
    current_length
        .checked_add(chunk_length)
        .filter(|length| *length <= MAX_RELEASE_MANIFEST_BYTES)
        .ok_or(ManifestBodyError::TooLarge)
}

fn validate_complete_manifest_length(
    length: usize,
) -> std::result::Result<(), ManifestBodyError> {
    if length == 0 {
        Err(ManifestBodyError::Empty)
    } else if length > MAX_RELEASE_MANIFEST_BYTES {
        Err(ManifestBodyError::TooLarge)
    } else {
        Ok(())
    }
...
fn manifest_body_error(error: ManifestBodyError) -> Error {
    let message = match error {
        ManifestBodyError::Empty => "Updater release manifest response body was empty.",
        ManifestBodyError::TooLarge => {
            "Updater release manifest response body exceeded the 1 MiB limit."
        }
    };
    Error::Network(message.into())
}

async fn read_release_manifest(response: reqwest::Response) -> Result<serde_json::Value> {
    let content_length = response.content_length();
    validate_declared_manifest_length(content_length).map_err(manifest_body_error)?;
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk?;
        checked_manifest_length(body.len(), chunk.len()).map_err(manifest_body_error)?;
        body.extend_from_slice(&chunk);
    }
    validate_complete_manifest_length(body.len()).map_err(manifest_body_error)?;
            ...
            match response {
                Ok(res) => {
                    if res.status().is_success() {
                        ...
                        let update_response = read_release_manifest(res).await?;
...
mod tests {
    use super::{
        ManifestBodyError, MAX_RELEASE_MANIFEST_BYTES, checked_manifest_length,
        validate_complete_manifest_length, validate_declared_manifest_length,
    };

    #[test]
    fn release_manifest_rejects_empty_bodies() {
        assert_eq!(
            validate_declared_manifest_length(Some(0)),
            Err(ManifestBodyError::Empty)
        );
        assert_eq!(
            validate_complete_manifest_length(0),
            Err(ManifestBodyError::Empty)
        );
    }

    #[test]
    fn release_manifest_rejects_declared_and_streamed_overflow() {
        assert_eq!(
            validate_declared_manifest_length(Some(MAX_RELEASE_MANIFEST_BYTES as u64 + 1)),
            Err(ManifestBodyError::TooLarge)
        );
        assert_eq!(
            checked_manifest_length(MAX_RELEASE_MANIFEST_BYTES, 1),
            Err(ManifestBodyError::TooLarge)
        );
        assert_eq!(
            checked_manifest_length(usize::MAX, 1),
            Err(ManifestBodyError::TooLarge)
        );
    }
