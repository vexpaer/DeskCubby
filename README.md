...
| **Structured records** | Replace plain daily templates with typed field values (word / number / type / time / duration) embedded in your Markdown diary via stable hidden comments. The same `.deskcubby/records.json` templates are available from Home, the diary editor and Android home-screen widgets; widget submissions keep the same structured Markdown markers. A `.deskcubby/` index powers per-field statistics, formula-built derived metrics and optional on-device auto wake/sleep estimation. |
