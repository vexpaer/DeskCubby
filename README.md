...
Current versions: Android **0.13.2** and Windows **0.7.0**.

The repository is split by platform: the complete Android project lives in `android/` and the Windows project in `windows/`. Public project documentation, including `README.md`, `README.zh-CN.md`, `TUTORIAL.md`, and the license, remains at the repository root.

## Windows 0.7.0

The Windows client supports Windows 10/11 x64. Version 0.7.0 accepts Android data formats v1–v29 and always exports v29. On wide layouts, the sidebar scrolls independently at every window height while Settings stays pinned at the bottom; collapsing a group also collapses its page buttons, and narrow windows can still turn the sidebar into a drawer. The UI supports Chinese and English, system light/dark mode, font scaling, and the Material, Liquid Glass, and Organic Future themes. The app logo is a transparent 512×512 pixel-art image, with the full Windows icon set generated through nearest-neighbor scaling. PDFs use the MIT-licensed React-PDF 10.4.1 viewer on Mozilla PDF.js 5.4.296 because WebView2 does not include a PDF viewer.
...
- Reading provides a local TXT/PDF bookshelf, progress, floating table of contents, full-book search, and a full-height reading surface with one compact toolbar. TXT now applies its chosen foreground to every paragraph, enforces readable contrast, and adds first-line indent, letter spacing, and page-padding controls. React-PDF supplies selectable text, annotation links, embedded PDF outlines, password prompts, rotation, continuous virtualized scrolling or single-page flow, and bundled CMaps/standard fonts/ICC/WASM support. PDFs default to their original colors; an explicit monochrome Reader Colors mode maps the rendered canvas to the selected background/foreground without modifying the file. PDF bytes are still fetched on demand through the UUID-scoped read-only protocol, and absolute paths never cross into React. Reader IPC is v3 and private state schema v4 losslessly upgrades v1–v3. Full-file fingerprints remain identical to Android, and each cloud configuration can independently synchronize progress through `reading/v1/progress.json`.
...
### Windows 0.7.0 platform boundaries
...
- Media on standalone Markdown lines can be reordered by dragging. In source mode, ordinary prose now uses the full editor width; only a standalone media line is locally covered at its right edge by the delete/drag controls. Both source and preview provide media delete controls; after confirmation they remove every reference to that file from the current journal, remove the media file, and best-effort clean its sidecar metadata. SHA-256 external-edit checks still run first, and a failed primary journal or media action is never reported as successful.
...
- Optional Distraction-free Fullscreen expands content into safe areas and initially hides the book title, toolbar, page indicator, and system bars. Tapping the center shows or hides controls without moving or resizing the page beneath it; visible chrome overlays the top of the existing reading plane. Orientation can follow the system, lock portrait, or lock landscape.
...
- Per-book URI, cover, page/paragraph position, and global reading preferences are stored in a private schema-v7 JSON file compatible with v1–v6. v7 saves the first visible TXT or PDF page plus a page-local checkpoint floored to 5% increments, so a position such as page 13.65 resumes within that page without skipping unread content. Stable scroll positions are debounced for roughly 600 ms and checkpointed again when the reader closes. Foreground reading time stays in the separate private `engagement-times-v1.json`; its internal schema v2 also stores the last known title for every timed book, upgrades v1 totals without loss, and keeps both title and duration after the book leaves the shelf. This page-local offset and engagement history remain Android-private: v29 application JSON and `reading/v1/progress.json` keep their existing URI-free page/paragraph field set and project the offset to the start of the recorded page. A SHA-256 fingerprint combines all book bytes and its type; title, URI, cover, content, bookshelf entries, display preferences, and reading time remain excluded from those progress exports and Android system backup.
...
0.13.2 (2026-08-12) restores the full Markdown source width for ordinary journal text while keeping delete/drag controls locally over standalone media rows. Reader chrome now overlays the page when toggled in Distraction-free Fullscreen, so center text does not jump when the title bar, tools, page indicator, or system bars appear. Private engagement JSON moves to schema v2, retains every timed book's last known title after shelf removal, and upgrades v1 duration totals without loss. Android moved to versionCode 29; Reader state remains schema v7, backup remains v29, Room remains v12, and Windows remained 0.6.0 at the time of that Android release.

0.13.1 (2026-08-12) makes Android TXT and PDF resume positions precise to 5% of a page. The first visible page and a floored 0/5/…/95 checkpoint are saved after roughly 600 ms of stability and again when leaving, so resuming does not jump past unread content. Reader private state moved to schema v7 while retaining v1–v6 compatibility. The finer offset stays local to Android; v29 and `reading/v1/progress.json` retain their existing privacy-bounded page/paragraph schema. Android moved to versionCode 28; backup remains v29, Room remains v12, and Windows remained 0.6.0 at the time of that Android release.
...
Windows 0.7.0 (2026-08-13) replaces the fragile hand-written one-canvas PDF renderer with the MIT-licensed React-PDF 10.4.1 layer on PDF.js 5.4.296. It removes the `pageColors` render option that reduced real pages to one solid color in WebView2, preserves the original document by default, and bundles its worker, CMaps, standard fonts, ICC profiles, and WASM decoders for offline use. PDF reading now includes selectable text, safe annotation links, embedded outlines, whole-document text search, rotation, session-only password entry, continuous virtualized vertical scrolling, and single-page mode. Only pages near the viewport keep live canvases, so long documents do not render every page at once.

The same release fixes global paragraph CSS overriding the TXT foreground, adds a live color preview and a 4.5:1 minimum contrast fallback, and makes both background and foreground colors persistently adjustable. Reader settings add PDF original/reader-color modes, continuous/single-page flow, page gaps, TXT first-line indent, letter spacing, and page padding. Reader IPC moves to v3 and private state to schema v4 with lossless defaults for v1–v3; SQLite remains v7 and the Android exchange format remains v29. Windows becomes 0.7.0 while Android remains 0.13.2.

Windows 0.6.1 (2026-08-13) fixes PDFs being rejected before pdf.js could load them: the Rust tagged-enum serializer emitted `asset_url` while the v1 TypeScript contract read `assetUrl`. Reader IPC v2 now pins that wire name explicitly and tests the serialized payload, while runtime URL validation and CSP continue to accept only DeskCubby's UUID-scoped `reader:` or `http://reader.localhost` read-only forms—never absolute paths. The now-unused frame source is closed. The reading surface is full height with a single compact toolbar, floating table of contents, denser shelf, keyboard paging, temporary PDF zoom, and an overlay focus mode that does not reduce text or canvas space.

The same release adds list/grid shelf layout, optional shelf progress and grid titles, six page backgrounds, custom foreground color, serif/sans/monospace type, natural/justified alignment, adjustable 520–1280 px text width, and a saved default focus mode alongside the existing font, spacing, PDF zoom, and chapter controls. Reader private state moves to schema v3 with lossless v1/v2 defaults; book paths, titles, content, preferences, and reading time remain local. Windows became 0.6.1; SQLite stays v7 and the exchange format stays v29.
...
- Portable test executable and checksum: `windows/artifacts/DeskCubby-0.7.0-windows-x64-portable.exe` and its `.sha256`
