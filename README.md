...
Current versions: Android **0.13.2** and Windows **0.6.0**.
...
0.13.2 (2026-08-12) restores the full Markdown source width for ordinary journal text while keeping delete/drag controls locally over standalone media rows. Reader chrome now overlays the page when toggled in Distraction-free Fullscreen, so center text does not jump when the title bar, tools, page indicator, or system bars appear. Private engagement JSON moves to schema v2, retains every timed book's last known title after shelf removal, and upgrades v1 duration totals without loss. Android moved to versionCode 29; Reader state remains schema v7, backup remains v29, Room remains v12, and Windows remains 0.6.0.

0.13.1 (2026-08-12) makes Android TXT and PDF resume positions precise to 5% of a page. The first visible page and a floored 0/5/…/95 checkpoint are saved after roughly 600 ms of stability and again when leaving, so resuming does not jump past unread content. Reader private state moved to schema v7 while retaining v1–v6 compatibility. The finer offset stays local to Android; v29 and `reading/v1/progress.json` retain their existing privacy-bounded page/paragraph schema. Android moved to versionCode 28; backup remains v29, Room remains v12, and Windows remains 0.6.0.
