...
当前版本：Android **0.12.0**；Windows **0.5.0**。

仓库按平台拆分：完整 Android 工程位于 `android/`，Windows 工程位于 `windows/`；`README.md`、`TUTORIAL.md`、`overview.md`、许可证等项目级文档继续保留在仓库根目录。

## Windows 0.5.0

Windows 客户端支持 Windows 10/11 x64。0.5.0 支持 Android v28 数据格式，左侧竖栏在任何宽屏高度都可独立滚动，底部设置入口始终固定可达；分组标题会连同下面的页面按钮一起折叠，窄窗口仍可收起为抽屉。界面同时支持中文/英文、系统明暗模式、字号缩放，以及 Material、Liquid Glass、Organic Future 三套主题。应用 Logo 使用透明 512×512 像素画，并以最近邻缩放生成整套 Windows 图标。Android 0.12.0 已把备份升级为 v29；Windows 0.5.0 暂不接受这一未来格式，请勿用它导入 Android 0.12.0 新导出的完整应用 JSON。
...
- 首页组件可增删、排序和单独控制标题显示，并提供小巧思、日常记录、饮食图片、笔记入口、小游戏直达、日记/小巧思/日期记录概览，以及真正位于应用首页的“云端同步”模块；该模块显示队列、进度、上次完成时间和待确认应用 JSON，可执行立即同步，并在确认后执行强制上传或单来源强制下载。“设置 → 子页面设置 → 主页 → 小游戏快捷入口”可从三种 2048、贪吃蛇、俄罗斯方块、扫雷、蜘蛛纸牌和围棋中任意选择。左上角默认提供 24 条简短中性问候，按日期稳定轮换并完整显示。“设置 → 子页面设置 → 主页 → 主页问候”可修改用户名、复制 `{name}` 占位符，以及增加、修改或删除中英文问候语。
...
- 首次启动的预设仍保持简洁：底栏只有主页、日记、小巧思、导航和设置；主页默认显示“今天、每日诗词、快速输入、饮食图片、年度进度、笔记、小游戏、记录概览、云端同步”九个模块。旧用户升级时也会把云端同步模块追加一次；之后仍可在主页设置中移除、排序或隐藏标题，其余模块和页面都可随时添加。
- 设置子页使用右上角保存，并可用重置按钮把本页全部草稿恢复为默认值后再保存；存在未保存修改时返回会先弹出确认提示。
- 应用冷启动开屏底色固定为黑色，避免系统启动窗口先闪白。
- “导航 → 小卡片”设计页可保存多套桌面信息组件，设置 1–6 格的自定义宽高、常用尺寸、背景/文字颜色和 SAF 背景图片，并显示主页模块或启动另一应用。0.12.0 增加名称显隐、0%–100% 背景透明度、图标显隐、左/中/右文字对齐和 75%–150% 字号；每个系统桌面实例按自己的 App Widget ID 保存完整设计快照，添加多个实例不再一起变成同一张卡，长按桌面实例进入重新配置时可另选设计。配置完成会立即写入首帧，前台更新广播之外还有 WorkManager 即时与约 6 小时保底刷新；除信息卡外，系统小组件面板还可添加单按钮“立即同步”和左右分区的“强制上传 / 强制下载”。最终是否发现、允许固定、采用格数和安排后台刷新仍由 Android 系统与当前启动器决定。
...
- “阅读”可从导航/More 页面进入，通过 SAF 导入 TXT 或 PDF；只保存持久读取授权和阅读状态，不复制或改写原书文件。TXT 支持 UTF-8、UTF-16 与 GB18030 回退解码；PDF 在 Android 9+ 会先确认安装包中的 AndroidX PDF 隔离服务存在，等 `PdfView` 附着后绑定文档，并同时以首屏内容和页面位图确认可用性。0.12.0 不再把首屏前可能出现、但不代表文档打不开的 AndroidX `RequestFailureEvent` 一律判成致命失败；文档打开失败、绑定异常、服务不可用或首屏等待超过 30 秒时仍自动切换到基于 `PdfRenderer` 的连续兼容视图，并可从提示条重试增强视图。Android 8 始终使用兼容视图。增强能力仍取决于系统与 PDF 文档本身，失败回退不会损坏或改写原文件。
...
- 应用首页的“云端同步”模块复用同一状态与串行 WorkManager 队列，显示是否启用、处理进度、上次完成时间和待确认应用 JSON；可直接立即同步，强制上传/下载则先显示与云同步设置页一致的影响确认。强制下载仍只在恰好一个来源启用时可用。
...
- 当前备份格式为 v29，最大 64 MiB，并继续安全导入 v1–v28。v29 在 v28 的受控 Custom 主题和最多 500 条 URI-free 阅读进度基础上，为每个桌面信息卡增加名称显隐、0%–100% 背景透明度、图标显隐、文字对齐和 75%–150% 字号；导入旧版时使用显示名称/图标、100% 背景、不缩放且左侧对齐的兼容默认。进度仍只含书籍 SHA-256 指纹、TXT/PDF 类型、位置、总页数和更新时间，不含书名、URI、封面或正文。Windows 0.5.0 只接受到 v28，暂不能导入 Android 0.12.0 导出的 v29。
...
- Windows 0.5.0 可导入 Android v1–v28，并统一导出 v28，但不会直接打开或共享 Android Room 数据库。Android 0.12.0 新导出的完整应用备份是 v29，Windows 0.5.0 会把它作为未来版本拒绝；日记、媒体和笔记真实文件仍应通过用户选择的普通目录互操作。
