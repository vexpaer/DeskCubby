...
当前版本：Android **0.9.0**；Windows **0.2.0**。
...
- 手机使用时间：Windows 0.2.0 **只显示、不采集**，但它只认识旧版 Android 手动导出的规范 v4 文件。Android 0.9.0 使用 v26/按设备云同步，因此此 Windows 页面暂不能接收 Android 0.9.0 的新数据；需等待后续 Windows 版本兼容。
...
### Windows v18 数据兼容（与 Android v26 的限制）

- Windows 0.2.0 只直接导入/导出 Android v18 JSON。Android 0.9.0 当前导出 v26，并在 v25 内容上新增笔记库 URI 引用、H1–H6 Markdown 标题字号与三项主页模块；Windows 0.2.0 **不能直接导入 v26**，本次 Android 更新不为旧 Windows 增加兼容层。
...
Windows 0.2.0 仍不实现浏览器、RSS、AI 聊天或热量估算、阅读、小游戏和健康统计。手机使用时间只保留旧 Android v4 文件的只读展示，不采集 Windows 或手机数据；它尚不理解 Android v26 的按设备历史。浏览器/RSS/AI 等 Android 数据继续在兼容影子中无损保留，不由 Windows 界面执行。
...
- 首页组件可增删、排序和单独控制标题显示，并提供小巧思、日常记录、饮食图片、笔记入口、2048/贪吃蛇/扫雷直达和日记/小巧思/日期记录概览等模块；左上角默认提供 24 条简短中性问候，按日期稳定轮换并完整显示。“设置 → 子页面设置 → 主页 → 主页问候”可修改用户名、复制 `{name}` 占位符，以及增加、修改或删除中英文问候语。
...
- “阅读”可从导航/More 页面进入，通过 SAF 导入 TXT 或 PDF；只保存持久读取授权和阅读进度，不复制或改写原书文件。TXT 支持 UTF-8、UTF-16 与 GB18030 回退解码，PDF 使用 Android `PdfRenderer` 按页显示。
- TXT 智能规则会识别更多中英文章节/卷/回/幕、Chapter/Part/Book/Section/Episode、罗马数字、序章/尾声、数字标题、Markdown 标题和括号标题；阅读设置可选“仅智能 / 仅自定义 / 智能 + 自定义”，输入匹配整行的正则，并调整章节标题最长字符数。规则变化会立即重新派生目录；即使没有匹配，也可继续使用稳定逻辑页、页码或进度跳转。PDF 同样支持页码/进度跳转。
- 默认采用滑动阅读。TXT 可调整 5 种预设背景或任意自定义颜色、12–38sp 字号、1.0–2.4 倍行距和 0–36dp 段距；方向可选跟随系统、锁定竖屏或锁定横屏。PDF 采用横向翻页并同样支持背景和方向设置。
- 每本书的阅读时长和页/段进度，以及全局阅读显示偏好与章节规则写入应用私有 schema-v3 JSON，并兼容 v1/v2。前台阅读时每 30 秒检查点保存，离开或退到后台时再次保存。状态文件采用回读校验和可恢复的 pending 提交；损坏、超限或提交失败时保护原文件并暂停后续写入。阅读书库、进度、显示偏好和时长都排除 DeskCubby v26 JSON 与 Android 系统备份。
...
- 收藏夹页面提供密码保护的私密文本收藏：首次使用设置密码，条目用密码派生密钥（PBKDF2 + AES-GCM）加密后保存在 Room。新密码允许 1 个到任意多个 Unicode 码点；密码不可找回。v26 会原样备份 AES-GCM 密文、IV、盐、迭代次数和加密校验值，但绝不写入密码、明文或派生密钥，因此换设备后输入原密码即可解锁。卡片不显示日期；有备注时直接显示备注，无备注时省略该行。普通正文单击复制，安全 HTTP(S) 链接单击用系统浏览器打开，长按进入含复制/编辑/删除的界面，右侧四点手柄可拖动排序；“设置 → 子页面设置 → 收藏夹”可调整卡片最小行高，最低 48dp，与小巧思的紧凑高度一致。
...
- 每个小游戏分别累计前台游玩总时长；计时每 30 秒检查点保存，并在离开游戏或应用退到后台时保存到应用私有 `engagement-times-v1.json`。总时长不会进入 Room 游戏存档、v26 JSON 或 Android 系统备份；特色统计存入 Room，并进入 v26 显式备份。
...
- 每次安装会生成不依赖硬件标识的随机稳定设备 ID，并使用可编辑的设备名称。手机使用时间页可选择“所有设备”或任一设备；本机只采集自己的系统数据，其他设备历史来自 v26 导入或云同步。“所有设备”按日期和应用相加，并显示短设备 ID 以区分同名设备。开启后每天首次打开应用也会尝试采集一次。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v26 JSON/自动备份。v26 还包含 Markdown 标题字号、笔记目录引用、桌面小卡片设计、多设备使用时间、小游戏存档与特色统计，以及 Vault 密文/校验元数据；请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。
...
- 选择“应用 JSON”会把 v26 备份以 `json/dc.json` 上传到所选服务，其中包含明文 AI API Key、Markdown 标题字号、笔记目录引用、桌面小卡片设计、多设备使用时间、小游戏存档与特色统计和 Vault 密文/校验元数据。另选“多设备使用时间”会使用每设备独立对象自动合并，无需恢复整份应用 JSON。HTTPS 保护传输过程，但不提供远端对象的端到端加密。
...
- 当前备份格式为 v26，最大 64 MiB。v26 新增笔记库 `content://` URI 引用、H1–H6 Markdown 标题字号与“笔记 / 小游戏 / 记录概览”主页模块；v25 的全局背景参数和教学总开关、v24 的音乐频率/小游戏特色统计、v23 的阅读导航项/游戏存档，以及 v22 的桌面小卡片设计继续保留。笔记、日记、媒体和背景图片文件本身都不嵌入 JSON；跨设备恢复时，没有本机持久读取授权的目录或图片 URI 会被安全丢弃。
- Vault 密码/明文/派生密钥、WebDAV 密码、S3 用户名/Key/Session Token、AI 对话历史及冻结上下文、笔记/日记正文、媒体文件、背景图片文件、阅读书库/进度、阅读与小游戏总时长、健康历史和系统权限不进入 JSON。v26 导入后 Vault 保持锁定；小游戏同 ID 最高分取较大值，较新存档胜出；特色统计逐项取较大值，使用时间按设备和日期合并。本机设备 ID 不会被导入文件覆盖。
- v26 仍包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和健康统计，云同步也保持关闭；音乐可视化设置会恢复，但录音权限必须由设备本机重新授予。Android 继续支持 v1–v25；旧备份没有新增字段时使用安全默认值。
...
- Windows 0.2.0 与 Android 旧版共用 v18 结构化 JSON，但不会直接打开或共享 Android Room 数据库。Android 0.9.0 当前导出 v26，Windows 0.2.0 尚不能直接导入；日记和媒体真实文件仍应通过用户选择的普通目录互操作。
- Windows 只保证直接导入 v18。Windows 重新导出会合并 DPAPI 加密影子中的未实现模块与未知字段，但这一机制不是编辑这些字段的界面。
- Windows 0.2.0 没有浏览器、RSS、AI/热量估算、阅读、小游戏或健康统计；其手机使用时间页只读取旧 Android v4 文件，不采集 Windows 使用时间，也尚未兼容 Android 0.9.0 的 v26/按设备云同步。
