...
- 首页组件可增删、排序和单独控制标题显示，并提供小巧思、日常记录、饮食图片、笔记入口、小游戏直达和日记/小巧思/日期记录概览等模块；“设置 → 子页面设置 → 主页 → 小游戏快捷入口”可从三种 2048、贪吃蛇、俄罗斯方块、扫雷、蜘蛛纸牌和围棋中任意选择。左上角默认提供 24 条简短中性问候，按日期稳定轮换并完整显示。“设置 → 子页面设置 → 主页 → 主页问候”可修改用户名、复制 `{name}` 占位符，以及增加、修改或删除中英文问候语。
...
- “导航 → 小卡片”设计页可保存多套桌面信息组件，设置 1–6 格的自定义宽高、常用尺寸、背景/文字颜色和 SAF 背景图片，并显示主页模块或启动另一应用。配置完成会立即写入首帧，前台更新广播之外还有 WorkManager 即时与约 6 小时保底刷新。0.11.0 为三个 provider 补齐显式启用、图标、名称和元数据，并提供 Android 12+ 描述/预览，改善通用系统与启动器的组件发现；除信息卡外，系统小组件面板还可添加单按钮“立即同步”和左右分区的“强制上传 / 强制下载”。最终是否发现、允许固定、采用格数和安排后台刷新仍由 Android 系统与当前启动器决定。
...
- “阅读”可从导航/More 页面进入，通过 SAF 导入 TXT 或 PDF；只保存持久读取授权和阅读状态，不复制或改写原书文件。TXT 支持 UTF-8、UTF-16 与 GB18030 回退解码；PDF 在 Android 9+ 会先确认安装包中的 AndroidX PDF 隔离服务存在，等 `PdfView` 附着后绑定文档，并同时以首屏内容和页面位图确认可用性。文档或首屏等待超过 30 秒、服务不可用或渲染请求失败时自动切换到基于 `PdfRenderer` 的连续兼容视图，并可从提示条重试增强视图；Android 8 始终使用兼容视图。增强能力仍取决于系统与 PDF 文档本身，失败回退不会损坏或改写原文件。
...
- 书架右上角设置可在现有列表与两列封面间切换，并独立决定是否显示阅读进度百分比。0.11.0 修复两列模式进入时的封面内存崩溃：封面按卡片实际宽度限制解码、输出像素与缓存大小，不再为书架中的多个任意 PDF 在应用进程中逐个打开渲染；PDF 优先使用已验证缓存或系统文档提供方的安全缩略图，无法取得时显示类型占位。PDF/TXT 都可选择一张 SAF 图片手动覆盖，移除后恢复安全缩略图或占位，不改动原书。
...
- 小游戏页面提供相互独立存档的 4×4、5×5、6×6 三种 2048、贪吃蛇、俄罗斯方块、自定义扫雷、横屏蜘蛛纸牌和本地双人围棋。围棋可选择 9/13/19 路棋盘，执行提子、禁自杀与简单劫；双方连续两次停着后结束，页面显示双方提子数，但不自动按地域或面积判定胜负。其他页面及游戏内未单独设色的文字统一使用当前主题前景色，能随亮/暗模式切换黑/白；2048 默认色板也跟随应用明暗模式，用户手动选择白天/黑夜后则保留该覆盖。2048 另支持慢速/标准/快速三档动画速度、分项动画开关、全页四向滑动和无限撤回；五位数及更长方块值会按位数与格宽单行缩放，绝不换行。扫雷可设置 6–30 行、6–30 列和合法范围内的雷数；单花色蜘蛛纸牌支持发牌、移动、自动收走 K→A、撤回和横屏布局。
- 小游戏会按各自玩法累计特色统计：2048 同时记录“总操作次数”（每次被游戏接受的方向输入，包含没有移动棋盘的输入）和“有效移动”，另有合并、最高方块及获胜；0.10.0 起不再新增或展示 2048 失败次数/胜率，但旧 Room/备份中的失败字段继续兼容往返。围棋记录落子、提子、停着和完成棋局数，其他游戏继续记录各自的食物、消行、扫雷、移动与胜负。统一“统计”页面汇总日记、手机使用时间、健康、阅读和全部游戏，卡片可进入带趋势图或指标图的统计子页。围棋存档、最高提子、特色统计和主页围棋快捷入口当前只保存在 Android 本机，并从 v28 备份投影排除；既有七个游戏/变体继续按原规则进入 v28。
- 每个小游戏分别累计前台游玩总时长；计时每 30 秒检查点保存，并在离开游戏或应用退到后台时保存到应用私有 `engagement-times-v1.json`。总时长不会进入 Room 游戏存档、v28 JSON 或 Android 系统备份；特色统计存入 Room，既有七个游戏/变体进入 v28 显式备份，围棋统计只留在 Android 本机。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v28 JSON/自动备份。v28 还包含自定义主题、Markdown 标题字号、笔记目录引用、既有七个游戏/变体的主页快捷入口、存档与特色统计、桌面小卡片设计、多设备使用时间、URI-free 阅读进度，以及 Vault 密文/校验元数据；请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。围棋快捷入口、存档和统计当前不进入 v28。
...
- S3 接入点可省略协议：SSL/TLS 默认开启并自动补 `https://`，仅可信内网可关闭后使用 HTTP。S3 支持默认开启的 Path-Style（`/Bucket/目录`）与 Bucket 子域名寻址，可用于 CSTCloud 等兼容端点。部分服务返回未加引号、弱、多个或缺失 ETag 时，0.11.0 使用与实际校验读取相同方法的**有界条件 GET**发送故意不匹配的 `If-Match` 探针，不再根据条件 HEAD 推断 GET 语义；该故意不匹配探针固定最多读取 64 KiB，超限会映射为 `SYNC_REMOTE_VALIDATION`，不会为判断条件语义再缓冲一份大对象。只有探针被拒绝、后续候选条件 GET 与内容 SHA-256 一致时才修复或采用验证器。不支持条件请求的服务仍失败关闭，两台手机的过期写入仍会转为冲突而不是覆盖。
...
- Vault 密码/明文/派生密钥、WebDAV 密码、S3 用户名/Key/Session Token、AI 对话历史及冻结上下文、笔记/日记正文、媒体文件、背景图片文件、阅读书架/封面/偏好、阅读与小游戏总时长、健康历史和系统权限不进入 JSON。v28 导入后 Vault 保持锁定；既有七个游戏/变体同 ID 最高分取较大值，较新存档胜出，特色统计逐项取较大值；0.11.0 的围棋存档、最高提子、特色统计和主页围棋快捷入口从 v28 投影排除，只保存在 Android 本机。使用时间按设备和日期合并，本机设备 ID 不会被导入文件覆盖。
...
- WebDAV 远端目录需要预先存在，且仍需在普通响应或 `PROPFIND Depth: 0` 属性中提供单个合法强 ETag。S3 可在有界条件 GET 探针与同方法内容校验通过后兼容部分非标准/缺失 ETag；不会再因服务只忽略条件 HEAD 就误判其 GET 语义，但只要服务不可靠执行 `If-Match`，同步仍会失败关闭。
