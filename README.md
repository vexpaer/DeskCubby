...
| **Structured records** | Replace plain daily templates with typed field values (word / number / type / time / duration) embedded in your Markdown diary via stable hidden comments. A `.deskcubby/` index and configurable day boundary (default 05:00) power per-field statistics, formula-built derived metrics (e.g. sleep duration) and optional on-device auto wake/sleep estimation. |
| **Home-screen widgets** | Android cards with rounded or square corners, a consistent adjustable card scale across every 1×1/2×1/1×2/2×2/long layout, fixed-size adjustable app icons, full-bleed music visuals, screen-time charts, meal capture, quick thoughts, desktop games, reader progress and cloud sync — your layout, your colors. |
