...
当前版本：Android **0.7.0**；Windows **0.2.0**。
...
- 手机使用时间：Windows 0.2.0 **只显示、不采集**，但它只认识旧版 Android 手动导出的规范 v4 文件。Android 0.7.0 使用 v24/按设备云同步，因此此 Windows 页面暂不能接收 Android 0.7.0 的新数据；需等待后续 Windows 版本兼容。
...
### Windows v18 数据兼容（与 Android v24 的限制）

- Windows 0.2.0 只直接导入/导出 Android v18 JSON。Android 0.7.0 当前导出 v24，并在 v23 的阅读、音乐可视化和游戏存档字段上继续加入可视化频率模式与小游戏特色统计；Windows 0.2.0 **不能直接导入 v24**，本次 Android 更新不为旧 Windows 增加兼容层。
...
Windows 0.2.0 仍不实现浏览器、RSS、AI 聊天或热量估算、阅读、小游戏和健康统计。手机使用时间只保留旧 Android v4 文件的只读展示，不采集 Windows 或手机数据；它尚不理解 Android v24 的按设备历史。浏览器/RSS/AI 等 Android 数据继续在兼容影子中无损保留，不由 Windows 界面执行。
...
- 每本书的阅读时长和进度，以及全局阅读显示偏好写入应用私有 JSON；前台阅读时每 30 秒检查点保存，离开或退到后台时再次保存。状态文件采用回读校验和可恢复的 pending 提交；损坏、超限或提交失败时保护原文件并暂停后续写入。阅读书库、进度、显示偏好和时长都排除 DeskCubby v24 JSON 与 Android 系统备份。
...
- 收藏夹页面提供密码保护的私密文本收藏：首次使用设置密码，条目用密码派生密钥（PBKDF2 + AES-GCM）加密后保存在 Room。新密码允许 1 个到任意多个 Unicode 码点；密码不可找回。v24 会原样备份 AES-GCM 密文、IV、盐、迭代次数和加密校验值，但绝不写入密码、明文或派生密钥，因此换设备后输入原密码即可解锁。卡片不显示日期；有备注时直接显示备注，无备注时省略该行。普通正文单击复制，安全 HTTP(S) 链接单击用系统浏览器打开，长按进入含复制/编辑/删除的界面，右侧四点手柄可拖动排序；“设置 → 子页面设置 → 收藏夹”可调整卡片最小行高，最低 48dp，与小巧思的紧凑高度一致。
- 小游戏页面提供相互独立存档的 4×4、5×5、6×6 三种 2048、贪吃蛇、俄罗斯方块、自定义扫雷和横屏蜘蛛纸牌。2048 支持慢速/标准/快速三档动画速度、白天/黑夜、分项动画开关、全页四向滑动和无限撤回；扫雷可设置 6–30 行、6–30 列和合法范围内的雷数，首次点击安全，长按插旗，双击已翻开的数字会翻开它周围除旗子外的所有格子；单花色蜘蛛纸牌支持发牌、移动连续同花顺、自动收走 K→A、撤回和横屏布局。
...
- 每个小游戏分别累计前台游玩总时长；计时每 30 秒检查点保存，并在离开游戏或应用退到后台时保存到应用私有 `engagement-times-v1.json`。总时长不会进入 Room 游戏存档、v24 JSON 或 Android 系统备份；特色统计存入 Room，并进入 v24 显式备份。
...
- 每次安装会生成不依赖硬件标识的随机稳定设备 ID，并使用可编辑的设备名称。手机使用时间页可选择“所有设备”或任一设备；本机只采集自己的系统数据，其他设备历史来自 v24 导入或云同步。“所有设备”按日期和应用相加，并显示短设备 ID 以区分同名设备。开启后每天首次打开应用也会尝试采集一次。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v24 JSON/自动备份。v24 还包含桌面小卡片设计、多设备使用时间、小游戏存档与特色统计，以及 Vault 密文/校验元数据；请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。
...
- 选择“应用 JSON”会把 v24 备份以 `json/dc.json` 上传到所选服务，其中包含明文 AI API Key、桌面小卡片设计、多设备使用时间、小游戏存档与特色统计和 Vault 密文/校验元数据。另选“多设备使用时间”会使用每设备独立对象自动合并，无需恢复整份应用 JSON。HTTPS 保护传输过程，但不提供远端对象的端到端加密。
...
- 当前备份格式为 v24，最大 64 MiB。v24 新增音乐可视化频率模式/上下限与小游戏特色统计；继续包含 v23 的阅读导航项、底栏音乐可视化、2048 动画速度、扫雷/蜘蛛纸牌存档，以及 v22 的桌面小卡片设计。背景图片文件本身不嵌入 JSON；跨设备恢复时，没有本机持久读取授权的背景 URI 会被安全丢弃。
- Vault 密码/明文/派生密钥、WebDAV 密码、S3 用户名/Key/Session Token、AI 对话历史及冻结上下文、日记正文、媒体文件、卡片背景图片、阅读书库/进度、阅读与小游戏总时长、健康历史和系统权限不进入 JSON。v24 导入后 Vault 保持锁定；小游戏同 ID 最高分取较大值，较新存档胜出；特色统计逐项取较大值，使用时间按设备和日期合并。本机设备 ID 不会被导入文件覆盖。
- v24 仍包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和健康统计，云同步也保持关闭；音乐可视化设置会恢复，但录音权限必须由设备本机重新授予。Android 继续支持 v1–v23；旧备份没有新增字段时使用安全默认值。
...
- Windows 0.2.0 与 Android 旧版共用 v18 结构化 JSON，但不会直接打开或共享 Android Room 数据库。Android 0.7.0 当前导出 v24，Windows 0.2.0 尚不能直接导入；日记和媒体真实文件仍应通过用户选择的普通目录互操作。
- Windows 只保证直接导入 v18。Windows 重新导出会合并 DPAPI 加密影子中的未实现模块与未知字段，但这一机制不是编辑这些字段的界面。
- Windows 0.2.0 没有浏览器、RSS、AI/热量估算、阅读、小游戏或健康统计；其手机使用时间页只读取旧 Android v4 文件，不采集 Windows 使用时间，也尚未兼容 Android 0.7.0 的 v24/按设备云同步。
