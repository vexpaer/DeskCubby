...
- 首页组件可增删、排序和单独控制标题显示，并提供小巧思、日常记录、饮食图片等快捷模块；左上角问候有 24 套中英句式，按日期稳定轮换并代入昵称。
- 底部导航可修改名称、图标、显示状态、顺序、默认启动页和标签显示方式；设置入口不可隐藏。
- “导航/More”聚合页可收纳不想直接放在底栏的其他主页面。每个适用页面可分别选择“显示在底栏”和“放入导航页”，减少底栏拥挤；独立的“设置 → 导航页”还可总开关描述、编辑每页描述。聚合页使用左右列分别按真实卡片高度排列的双列瀑布流。
...
- 收藏夹页面提供密码保护的私密文本收藏：首次使用设置密码，条目用密码派生密钥（PBKDF2 + AES-GCM）加密后保存在本机 Room，支持解锁/锁定、编辑、删除和修改密码（全量重加密）。新密码至少 4 个 Unicode 码点且没有最大长度；密码不可找回，密文和口令元数据均不进入 JSON 备份。
- 小游戏页面内置 2048、贪吃蛇和俄罗斯方块，各自记录最高分；退出、应用进入后台或离开游戏页时自动暂停并保存进度，下次可继续或开新局。棋盘与方块颜色取自当前设置的主题主色与首个副色。

### 手机使用时间与步数

- 新增“手机使用时间”主页面，经 Android“使用情况访问权限”读取前台应用事件，按本地自然日记录总时长及每个应用的时长；可查看全部应用总计或切换到具体应用。
...
- 聊天输入区可逐项选择日记、小巧思、日期记录或诗词本条目作为上下文，并在发送前预览。每次最多 50 项、单项 64 KiB、总计 256 KiB；超限会拒绝发送而不会静默截断。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v15 JSON/自动备份。请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。
...
- 选择“应用 JSON”会把未加密的 v15 备份上传到所选服务，其中包含明文 AI API Key。HTTPS 保护传输过程，但不提供远端对象的端到端加密；手机使用时间和步数历史不会随之上传。
- 从云端下载的应用 JSON 只会校验并暂存在应用私有目录；后台任务不会直接覆盖本机设置或 Room，必须由用户在同步设置中确认后才会恢复。

### 备份

- 支持选择自动备份目录、立即保存、手动导入和导出单个 JSON 文件；“设置 → 应用数据与备份 → 查看整体 JSON”可查看当前完整备份快照，文本可滚动与长按选择，AI API Key 会按实际备份内容完整明文显示。
- 当前备份格式为 v15，包含普通应用设置（含紧凑模式、桌面名称与图标、吃历换行、重点颜色等）、导航聚合选择与自定义描述、吃历滤镜、云同步非秘密元数据、AI 配置及明文 API Key、小巧思（含重点标记）与分类、浏览器收藏、日期记录和诗词本。
- WebDAV/S3 凭据、AI 对话历史及冻结上下文、日记正文、媒体文件、收藏夹密文与口令元数据、小游戏存档、手机使用时间历史和步数历史不进入 JSON 备份；跨设备恢复后需要重新授予日记与媒体目录访问权限，并复核或重新填写云端凭据。
- v15 虽包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和步数统计，系统权限也不会迁移；用户必须在新设备上重新开启和授权。导入后云同步同样始终保持关闭；v14 及更早版本仍受支持。
...
0.3.1（2026-07-27）已完成 Kotlin 编译、140 个 JVM 测试（140/140 通过）、Android 仪器测试源码编译、Debug APK、Lint（0 错误）及签名 Release APK 构建；Release APK 已通过 `apksigner` v2 签名校验。当前构建环境没有连接设备，因此 Usage Access、Health Connect、后台日结、SAF 长图导出、双启动器图标和游戏后台自动暂停仍建议在真实设备上逐项验证。
...
- 手机使用时间来自 Android UsageStats 前台事件，口径可能与设备厂商的“屏幕时间”不同，且不会补算启用统计前的日期。
