...
Current versions: Android **0.13.2** and Windows **0.6.1**.
...
- Reading provides a local TXT/PDF bookshelf, progress, floating table of contents, search, and a full-height reading surface with one compact toolbar. Its settings now include list/grid shelf layouts, optional shelf progress and grid titles, six page backgrounds, automatic or custom foreground color, serif/sans/monospace type, natural/justified alignment, font size, line and paragraph spacing, 520–1280 px text width, PDF baseline zoom, chapter rules, and an optional default focus mode whose translucent controls overlay rather than resize the page. pdf.js renders PDFs in-app while bytes are fetched on demand through a restricted read-only protocol; the v2 Reader IPC fixes the PDF field name as `assetUrl` so valid protocol URLs cannot be lost at the Rust/TypeScript boundary. Full-file fingerprints are identical to Android, and each cloud configuration can independently synchronize progress for the same book through `reading/v1/progress.json`. Windows paths and Android URIs are never written into one another.
...
### Windows 0.6.1 platform boundaries
...
- Portable test executable and checksum: `windows/artifacts/DeskCubby-0.6.1-windows-x64-portable.exe` and its `.sha256`
