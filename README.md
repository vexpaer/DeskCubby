...
当前版本：Android **0.6.6**；Windows **0.2.0**。
...
- 手机使用时间：Windows 0.2.0 **只显示、不采集**，但它只认识旧版 Android 手动导出的规范 v4 文件。Android 0.6.6 使用 v23/按设备云同步，因此此 Windows 页面暂不能接收 Android 0.6.6 的新数据；需等待后续 Windows 版本兼容。
...
### Windows v18 数据兼容（与 Android v23 的限制）

- Windows 0.2.0 只直接导入/导出 Android v18 JSON。Android 0.6.6 当前导出 v23，并在 v22 的桌面小卡片等字段上继续加入阅读导航项、音乐可视化设置、2048 动画速度，以及扫雷和蜘蛛纸牌存档；Windows 0.2.0 **不能直接导入 v23**，本次 Android 更新不为旧 Windows 增加兼容层。
...
Windows 0.2.0 仍不实现浏览器、RSS、AI 聊天或热量估算、阅读、小游戏和健康统计。手机使用时间只保留旧 Android v4 文件的只读展示，不采集 Windows 或手机数据；它尚不理解 Android v23 的按设备历史。浏览器/RSS/AI 等 Android 数据继续在兼容影子中无损保留，不由 Windows 界面执行。
...
- 底部导航可修改名称、图标、显示状态、顺序、默认启动页和标签显示方式；设置入口不可隐藏。还可在同一设置页开启音乐可视化并选择直方图、波形或平滑曲线；取得 Android 录音权限后，可视化器只读取系统当前音频会话的频谱/波形用于实时绘制，不保存或上传音频。可视化层只匹配底栏自身高度，不参与页面测量；退到后台、权限缺失或系统关闭动画时会停止捕获。
...
- 默认采用滑动阅读。TXT 可调整 5 种背景、12–38sp 字号、1.0–2.4 倍行距和 0–36dp 段距；方向可选跟随系统、锁定竖屏或锁定横屏。PDF 采用横向翻页并同样支持横屏锁定。
- 每本书的阅读时长和进度，以及全局阅读显示偏好写入应用私有 JSON；前台阅读时每 30 秒检查点保存，离开或退到后台时再次保存。状态文件采用回读校验和可恢复的 pending 提交；损坏、超限或提交失败时保护原文件并暂停后续写入。阅读书库、进度、显示偏好和时长都排除 DeskCubby v23 JSON 与 Android 系统备份。
...
- 热量结果记录在媒体目录的 `dc-media.json`（兼容读取旧名 `deskcubby-media.json`，不再改写图片 Markdown 标题；旧版写在标题里的 `午餐-800kJ` 仍可读取）。点击日期后的总热量可打开完整详情，按“早餐 1 / 午餐 1 / 午餐 2”等列出食物明细；总热量可手动修改或恢复估算小计，并可保存仅在详情中显示的备注。重新计算会保留备注并把它发送给文字模型，适配同一份饭多次拍摄或多人聚餐。
...
- 收藏夹页面提供密码保护的私密文本收藏：首次使用设置密码，条目用密码派生密钥（PBKDF2 + AES-GCM）加密后保存在 Room。新密码允许 1 个到任意多个 Unicode 码点；密码不可找回。v23 会原样备份 AES-GCM 密文、IV、盐、迭代次数和加密校验值，但绝不写入密码、明文或派生密钥，因此换设备后输入原密码即可解锁。卡片不显示日期；有备注时直接显示备注，无备注时省略该行。普通正文单击复制，安全 HTTP(S) 链接单击用系统浏览器打开，长按进入含复制/编辑/删除的界面，右侧四点手柄可拖动排序；“设置 → 子页面设置 → 收藏夹”可调整卡片最小行高，最低 48dp，与小巧思的紧凑高度一致。
- 小游戏页面提供相互独立存档的 4×4、5×5、6×6 三种 2048、贪吃蛇、俄罗斯方块、自定义扫雷和横屏蜘蛛纸牌。2048 支持慢速/标准/快速三档动画速度、白天/黑夜、分项动画开关、全页四向滑动和无限撤回；扫雷可设置 6–30 行、6–30 列和合法范围内的雷数，首次点击安全，长按插旗；单花色蜘蛛纸牌支持发牌、移动连续同花顺、自动收走 K→A、撤回和横屏布局。
- 每个小游戏分别累计前台游玩总时长；计时每 30 秒检查点保存，并在离开游戏或应用退到后台时保存到应用私有 `engagement-times-v1.json`。总时长不会进入 Room 游戏存档、v23 JSON 或 Android 系统备份。
...
- 每次安装会生成不依赖硬件标识的随机稳定设备 ID，并使用可编辑的设备名称。手机使用时间页可选择“所有设备”或任一设备；本机只采集自己的系统数据，其他设备历史来自 v23 导入或云同步。“所有设备”按日期和应用相加，并显示短设备 ID 以区分同名设备。开启后每天首次打开应用也会尝试采集一次。
...
- Room v11 是手机使用时间、其他设备使用时间缓存和健康每日统计的唯一运行时权威来源。升级时会把 `usage-statistics.json`、旧设备缓存和 `step-statistics.json` 事务化、幂等迁入 Room；损坏文件原样保留且不会遮住其他有效文件。JSON 编解码只保留用于旧数据迁移、DeskCubby 备份/导入导出和云同步，外部格式保持兼容。为避免系统备份把同库中的健康明细带到其他设备，`deskcubby.db` 及其 WAL/SHM/journal 均排除 Android 系统备份；显式 DeskCubby JSON/云同步仍是受支持的迁移路径。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v23 JSON/自动备份。v23 还包含桌面小卡片设计、多设备使用时间、小游戏存档和 Vault 密文/校验元数据；请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。
...
- 选择“应用 JSON”会把 v23 备份以 `json/dc.json` 上传到所选服务，其中包含明文 AI API Key、桌面小卡片设计、多设备使用时间、小游戏存档和 Vault 密文/校验元数据。另选“多设备使用时间”会使用每设备独立对象自动合并，无需恢复整份应用 JSON。HTTPS 保护传输过程，但不提供远端对象的端到端加密。
...
- 当前备份格式为 v23，最大 64 MiB。v23 新增阅读导航项、底栏音乐可视化设置、2048 动画速度，以及扫雷/蜘蛛纸牌游戏 ID 与存档；继续包含 v22 的桌面小卡片设计、诗词排序、收藏夹行高、同步 User-Agent、Vault 密文和按设备分组的手机使用时间。背景图片文件本身不嵌入 JSON；跨设备恢复时，没有本机持久读取授权的背景 URI 会被安全丢弃。
- Vault 密码/明文/派生密钥、WebDAV 密码、S3 用户名/Key/Session Token、AI 对话历史及冻结上下文、日记正文、媒体文件、卡片背景图片、阅读书库/进度、阅读与小游戏总时长、健康历史和系统权限不进入 JSON。v23 导入后 Vault 保持锁定；小游戏同 ID 最高分取较大值，较新存档胜出；使用时间按设备和日期合并。本机设备 ID 不会被导入文件覆盖。
- v23 仍包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和健康统计，云同步也保持关闭；音乐可视化开关会恢复，但录音权限必须由设备本机重新授予。Android 继续支持 v1–v22；旧备份没有新增字段时使用安全默认值。
- 导入 v11 及更早备份时，仅为配置 ID 与 API 地址都一致的 AI 配置保留本机已有 Key。

### 本地数据库

- Room 数据库当前版本为 11，保留 1→2 至 10→11 的全部显式迁移，不使用 destructive migration；8→9 原样迁移旧诗词并增加诗词分类外键，9→10 按原“最新在前”顺序增加诗词持久排序，10→11 新增手机使用时间、外部设备缓存、健康每日统计和旧文件迁移标记表。
...
- Windows 0.2.0 与 Android 旧版共用 v18 结构化 JSON，但不会直接打开或共享 Android Room 数据库。Android 0.6.6 当前导出 v23，Windows 0.2.0 尚不能直接导入；日记和媒体真实文件仍应通过用户选择的普通目录互操作。
- Windows 只保证直接导入 v18。Windows 重新导出会合并 DPAPI 加密影子中的未实现模块与未知字段，但这一机制不是编辑这些字段的界面。
- Windows 0.2.0 没有浏览器、RSS、AI/热量估算、阅读、小游戏或健康统计；其手机使用时间页只读取旧 Android v4 文件，不采集 Windows 使用时间，也尚未兼容 Android 0.6.6 的 v23/按设备云同步。
