...
- 点击主页诗词模块可查看整首诗（含朝代），此时加入诗词本保存的是完整诗词；诗词本卡片长按后才显示编辑/删除操作。诗词本设置支持导入本机字体、调整字号与行距、选择对齐方式，以及控制出处和引号装饰显示。
...
- 新增“手机使用时间”主页面，经 Android“使用情况访问权限”分块读取按日 UsageStats；每次刷新会在安全上限内补采系统仍可访问且自然日边界完整的历史，并日结总时长及每个应用的时长。应用筛选优先读取系统应用标签和图标（例如把 `aweme` 显示为抖音），并按当前范围用时降序排列。
- 新增“步数记录”主页面，经用户授权从 Health Connect 只读聚合每日步数；设备不支持、没有数据、未授权或读取失败时不会伪造 0 步。
- 两页都提供“开始统计日期、已统计天数、总计、日均”总览，可选最近 7/30/90 天或全部范围。直方图、曲线、GitHub 风格格子图以同一行的纯图标切换，点击图中的某一天可查看该日数值；手机使用时间页不再显示“本机私有统计”说明卡。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v17 JSON/自动备份。请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。
...
- 选择“应用 JSON”会把未加密的 v17 备份以 `json/dc.json` 上传到所选服务，其中包含明文 AI API Key。HTTPS 保护传输过程，但不提供远端对象的端到端加密；手机使用时间和步数历史不会随之上传。
- 从云端下载的应用 JSON 只会校验并暂存在应用私有目录；后台任务不会直接覆盖本机设置或 Room，必须由用户在同步设置中确认后才会恢复。

### 备份

- 支持选择自动备份目录、立即保存、手动导入和导出单个 JSON 文件；自动备份使用 `dc.json`，手动导出默认使用 `dc-backup-日期.json`，并继续识别旧版 `DeskCubby*.json`。“设置 → 应用数据、备份与同步 → 查看整体 JSON”可查看当前完整备份快照。
- 当前备份格式为 v17，新增诗词本字体、字号、行距、对齐和装饰显示设置；它还包含普通应用设置（含主页问候、紧凑模式、桌面名称与图标、吃历换行、重点颜色等）、导航聚合选择与自定义描述、吃历滤镜、云同步非秘密元数据、AI 配置及明文 API Key、小巧思（含重点标记）与分类、浏览器收藏、日期记录和诗词本。
- WebDAV/S3 凭据、AI 对话历史及冻结上下文、日记正文、媒体文件、收藏夹密文与口令元数据、小游戏存档、手机使用时间历史和步数历史不进入 JSON 备份；跨设备恢复后需要重新授予日记与媒体目录访问权限，并复核或重新填写云端凭据。
- v17 仍包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和步数统计，系统权限也不会迁移；用户必须在新设备上重新开启和授权。导入后云同步同样始终保持关闭；v16 及更早版本仍受支持。
