...
- 小游戏页面内置相互独立存档的 4×4、5×5、6×6 三种 2048，以及贪吃蛇和俄罗斯方块，各自记录最高分；退出、应用进入后台或离开游戏页时自动暂停并保存进度，下次可继续或开新局。页面顶部安全区域与其他主页面一致；2048 使用差异更明显的主题色阶，并提供滑动、合并旋转回弹和新方块淡入动画，遵循系统动画时长设置。
