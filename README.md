...
0.3.4（2026-07-27）修复 0.3.2 引入的手机使用时间空记录回归：先按短区间发现 Android 仍保留数据的日期范围，再按本地自然日逐日读取，避免厂商返回跨日聚合时间戳时被全部过滤。旧版 usage-statistics v2 会保留已有日记录并自动重新执行一次正确回填。已通过 JVM 单元测试、Android 测试源码编译与 Debug 构建；因当前未连接设备，仍需在真实设备授权 Usage Access 后确认厂商数据。
