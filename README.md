...
当前版本：Android **0.6.0**；Windows **0.2.0**。
...
- 手机使用时间：Windows 0.2.0 **只显示、不采集**，但它只认识旧版 Android 手动导出的规范 v4 文件。Android 0.6.0 使用 v23/按设备云同步，因此此 Windows 页面暂不能接收 Android 0.6.0 的新数据；需等待后续 Windows 版本兼容。
...
- Windows 0.2.0 只直接导入/导出 Android v18 JSON。Android 0.6.0 当前导出 v23，并在 v22 的桌面小卡片等字段上继续加入阅读导航项、音乐可视化设置、2048 动画速度，以及扫雷和蜘蛛纸牌存档；Windows 0.2.0 **不能直接导入 v23**，本次 Android 更新不为旧 Windows 增加兼容层。
...
- 底部导航可修改名称、图标、显示状态、顺序、默认启动页和标签显示方式；设置入口不可隐藏。还可在同一设置页开启音乐可视化并选择直方图、波形或平滑曲线；取得 Android 录音权限后，可视化器只读取系统当前音频会话的频谱/波形用于实时绘制，不保存或上传音频。
...
- 每本书的阅读时长和进度，以及全局阅读显示偏好写入应用私有 JSON；前台阅读时每 30 秒检查点保存，离开或退到后台时再次保存。阅读书库、进度、显示偏好和时长都排除 DeskCubby v23 JSON 与 Android 系统备份。
...
- 本机历史仍保存在应用私有且排除 Android 系统备份的 `usage-statistics.json`；其他设备缓存、随机设备 ID 和健康历史也排除系统备份/设备迁移，防止克隆设备身份。手机使用时间会进入 v23 JSON，并可通过用户明确勾选的 WebDAV/S3“多设备使用时间”同步；步数、距离、活动热量历史和系统授权仍不进入 JSON 或云同步。
...
- 设置主页使用简短入口“设置 → 应用数据”；页面顶部会统计应用总占用和私有数据占用，并按安装包/代码、Room 数据库、DataStore/偏好、阅读数据、阅读与游戏时长、统计历史、缓存、其他私有文件、外部应用目录及用户选择的日记/媒体目录分组显示。SAF 目录扫描受数量和时间上限保护，未扫描完会明确标为估算值；可手动重新计算。
...
- 同步使用 SHA-256、远端 manifest、条件请求和冲突副本保护并发修改。WebDAV 优先使用强 ETag；服务未提供可用 ETag、但提供可解析的 `Last-Modified` 时，会把时间戳与对象 SHA-256 组合成版本，更新前预读完整对象、使用 `If-Unmodified-Since`，并在写入后回读验证。两种可靠验证器都没有时仍安全失败，不会无条件覆盖。当前不会传播删除：某一侧缺失的文件会按同步方向重新上传、下载或跳过。
...
应用数据页新增安装包、数据库、偏好、阅读、娱乐计时、统计、缓存、外部目录和 SAF 日记/媒体分组占用；收藏夹最低行高压缩到 48dp。WebDAV 在缺少强 ETag 时可使用 `Last-Modified` + SHA-256、更新前完整预读、`If-Unmodified-Since` 和写后回读的安全条件同步，缺少全部可靠验证器时仍失败关闭。应用 versionCode 为 16，备份为 v23，Room 仍为 v10；Windows 源码与版本未修改。
...
- Windows 0.2.0 与 Android 旧版共用 v18 结构化 JSON，但不会直接打开或共享 Android Room 数据库。Android 0.6.0 当前导出 v23，Windows 0.2.0 尚不能直接导入；日记和媒体真实文件仍应通过用户选择的普通目录互操作。
- Windows 只保证直接导入 v18。Windows 重新导出会合并 DPAPI 加密影子中的未实现模块与未知字段，但这一机制不是编辑这些字段的界面。
- Windows 0.2.0 没有浏览器、RSS、AI/热量估算、阅读、小游戏或健康统计；其手机使用时间页只读取旧 Android v4 文件，不采集 Windows 使用时间，也尚未兼容 Android 0.6.0 的 v23/按设备云同步。
