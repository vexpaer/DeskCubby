...
当前版本：Android **0.9.1**；Windows **0.2.0**。
...
- 手机使用时间：Windows 0.2.0 **只显示、不采集**，但它只认识旧版 Android 手动导出的规范 v4 文件。Android 0.9.1 使用 v27/按设备云同步，因此此 Windows 页面暂不能接收 Android 0.9.1 的新数据；需等待后续 Windows 版本兼容。
...
- Windows 0.2.0 只直接导入/导出 Android v18 JSON。Android 0.9.1 当前导出 v27；v26 已加入笔记库 URI、H1–H6 标题字号与三项主页模块，v27 再加入主页小游戏快捷入口选择。Windows 0.2.0 **不能直接导入 v27**，本次 Android 更新不为旧 Windows 增加兼容层。
...
- “吃历”按日期展示饮食照片，可设置图片高度上限和是否显示文字说明；点击照片可全屏放大查看（双指缩放、双击切换倍率），放大时显示热量与拍摄地点（如有）。每篇日记解析出的饮食图片引用会写入有界、可重建的应用缓存；URI、名称、修改时间和大小均未变化时不再重复读取 Markdown，手动刷新仍会强制重读。
...
- 配置文字模型与图片识别模型后，可开启上传图片自动估算热量：图片模型先识别食物名称与分量，再由文字模型返回总能量和逐项食物的名称、分量、单位及 kJ；默认两段提示词已加强，用户自定义提示词不会被覆盖。
- 吃历顶栏的“估算所有”按钮单击会把当前日期加入任务，长按直接进入热量估算进度页。进度页逐项显示日期、当天第几张图片以及“图片模型识别 / 文字模型估算 / 保存”阶段；点“正在处理”卡片可查看当前模型的实时用时、流式思考与回复，思考区域可折叠。不支持流式的兼容端点仍会回退到完整 JSON 回复。离开吃历后任务继续运行；其他日期可排队，同一天全部图片完成后只统一保存一次，失败保留明确阶段并继续后续任务。
- 热量结果记录在媒体目录的 `dc-media.json`（兼容读取旧名 `deskcubby-media.json`，不再改写图片 Markdown 标题；旧版写在标题里的 `午餐-800kJ` 仍可读取）。点击日期热量或详情入口可查看完整日期，按“早餐 1 / 午餐 1 / 午餐 2”等列出食物明细；没有结果的照片统一显示“估算失败”。每张照片旁的计算器可只重算该图并保留手工总量，整日重算则更新全部明细并清除手工总量；两者都会保留备注并把它发送给文字模型。
...
- Windows 0.2.0 与 Android 旧版共用 v18 结构化 JSON，但不会直接打开或共享 Android Room 数据库。Android 0.9.1 当前导出 v27，Windows 0.2.0 尚不能直接导入；日记和媒体真实文件仍应通过用户选择的普通目录互操作。
- Windows 只保证直接导入 v18。Windows 重新导出会合并 DPAPI 加密影子中的未实现模块与未知字段，但这一机制不是编辑这些字段的界面。
- Windows 0.2.0 没有浏览器、RSS、AI/热量估算、阅读、小游戏或健康统计；其手机使用时间页只读取旧 Android v4 文件，不采集 Windows 使用时间，也尚未兼容 Android 0.9.1 的 v27/按设备云同步。
