...
当前版本：Android **0.3.8**；Windows **0.2.0**。
...
### Windows v18 数据兼容（与 Android v19 的限制）

- Windows 0.2.0 只直接导入/导出 Android v18 JSON。Android 0.3.8 因新增诗词分类已升级为 v19：Android 仍可导入 v1–v18，但 Windows 0.2.0 **不能直接导入 Android 0.3.8 导出的 v19**，需等待 Windows 兼容版本。
...
- 手机使用时间页的「导出给 Windows」会先说明包名、每日前台时长和日期范围属于敏感习惯数据，再通过系统 SAF 创建 `dc-usage-statistics-v4.json`。它只导出已经采集的规范 v4 历史，写后会完整回读校验；不会开启统计、申请权限，也不会把明细加入 v19 或云同步。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v19 JSON/自动备份。请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。
...
- 选择“应用 JSON”会把未加密的 v19 备份以 `json/dc.json` 上传到所选服务，其中包含明文 AI API Key。HTTPS 保护传输过程，但不提供远端对象的端到端加密；手机使用时间和步数历史不会随之上传。
...
- 当前备份格式为 v19，新增诗词分类、诗词 `categoryId` 和 S3 Path-Style 元数据；它还包含普通应用设置、导航、吃历滤镜、云同步非秘密元数据、AI 配置及明文 API Key、小巧思与分类、浏览器收藏、日期记录和诗词本。
- WebDAV 密码、S3 用户名/Key/Session Token、AI 对话历史及冻结上下文、日记正文、媒体文件、收藏夹密文与口令元数据、小游戏存档、手机使用时间历史和步数历史不进入 JSON 备份；导入同 ID 且接入点、Bucket、Region 都一致的 S3 配置会保留当前设备已有的本机凭据，服务范围改变时不会静默复用。
- v19 仍包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和步数统计，系统权限也不会迁移；导入后云同步同样始终关闭。Android 继续支持 v1–v18，旧诗词会安全归入“未分类”。
...
- Windows 0.2.0 与 Android 旧版共用 v18 结构化 JSON，但不会直接打开或共享 Android Room 数据库。Android 0.3.8 当前导出 v19，Windows 0.2.0 尚不能直接导入；日记和媒体真实文件仍应通过用户选择的普通目录互操作。
