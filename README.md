...
当前版本：Android **0.5.0**；Windows **0.2.0**。
...
- 手机使用时间：Windows 0.2.0 **只显示、不采集**，但它只认识旧版 Android 手动导出的规范 v4 文件。Android 0.5.0 使用 v22/按设备云同步，因此此 Windows 页面暂不能接收 Android 0.5.0 的新数据；需等待后续 Windows 版本兼容。
...
### Windows v18 数据兼容（与 Android v22 的限制）

- Windows 0.2.0 只直接导入/导出 Android v18 JSON。Android 0.5.0 当前导出 v22，并加入桌面小卡片设计、诗词排序、同步 User-Agent、收藏夹行高、Vault 密文、小游戏存档和多设备使用时间；Windows 0.2.0 **不能直接导入 v22**，本次 Android 更新不为旧 Windows 增加兼容层。
...
Windows 0.2.0 仍不实现浏览器、RSS、AI 聊天或热量估算、小游戏和健康统计。手机使用时间只保留旧 Android v4 文件的只读展示，不采集 Windows 或手机数据；它尚不理解 Android v22 的按设备历史。浏览器/RSS/AI 等 Android 数据继续在兼容影子中无损保留，不由 Windows 界面执行。
...
- 收藏夹页面提供密码保护的私密文本收藏：首次使用设置密码，条目用密码派生密钥（PBKDF2 + AES-GCM）加密后保存在 Room。新密码允许 1 个到任意多个 Unicode 码点；密码不可找回。v22 会原样备份 AES-GCM 密文、IV、盐、迭代次数和加密校验值，但绝不写入密码、明文或派生密钥，因此换设备后输入原密码即可解锁。卡片不显示日期；有备注时直接显示备注，无备注时省略该行。普通正文单击复制，安全 HTTP(S) 链接单击用系统浏览器打开，长按进入含复制/编辑/删除的界面，右侧四点手柄可拖动排序；“设置 → 子页面设置 → 收藏夹”可调整卡片最小行高。
- 小游戏页面内置相互独立存档的 4×4、5×5、6×6 三种 2048，以及贪吃蛇和俄罗斯方块，各自记录最高分。2048 页面仅显示当前分数，不显示 2048.org 文案；页面任意位置均可靠识别上、下、左、右四向滑动，右下角回转箭头按钮用于无限撤回，棋盘更大且上下居中，并支持白天/黑夜和方块生成、移动、合并动画开关；数字字号按棋盘格大小适配。其他游戏继续记录各自最高分。
...
- 每次安装会生成不依赖硬件标识的随机稳定设备 ID，并使用可编辑的设备名称。手机使用时间页可选择“所有设备”或任一设备；本机只采集自己的系统数据，其他设备历史来自 v22 导入或云同步。“所有设备”按日期和应用相加，并显示短设备 ID 以区分同名设备。开启后每天首次打开应用也会尝试采集一次。
...
- 本机历史仍保存在应用私有且排除 Android 系统备份的 `usage-statistics.json`；其他设备缓存、随机设备 ID 和健康历史也排除系统备份/设备迁移，防止克隆设备身份。手机使用时间会进入 v22 JSON，并可通过用户明确勾选的 WebDAV/S3“多设备使用时间”同步；步数、距离、活动热量历史和系统授权仍不进入 JSON 或云同步。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v22 JSON/自动备份。v22 还包含桌面小卡片设计、多设备使用时间、小游戏存档和 Vault 密文/校验元数据；请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。
...
- 选择“应用 JSON”会把 v22 备份以 `json/dc.json` 上传到所选服务，其中包含明文 AI API Key、桌面小卡片设计、多设备使用时间、小游戏存档和 Vault 密文/校验元数据。另选“多设备使用时间”会使用每设备独立对象自动合并，无需恢复整份应用 JSON。HTTPS 保护传输过程，但不提供远端对象的端到端加密。
...
- 当前备份格式为 v22，最大 64 MiB。v22 新增桌面小卡片设计（尺寸、颜色、背景 URI、主页模块/应用按钮），继续包含诗词持久排序、收藏夹卡片行高、同步 User-Agent、Vault 密文、小游戏存档与按设备分组的手机使用时间。背景图片文件本身不嵌入 JSON；跨设备恢复时，没有本机持久读取授权的背景 URI 会被安全丢弃。
- Vault 密码/明文/派生密钥、WebDAV 密码、S3 用户名/Key/Session Token、AI 对话历史及冻结上下文、日记正文、媒体文件、卡片背景图片、健康历史和系统权限不进入 JSON。v22 导入后 Vault 保持锁定；小游戏同 ID 最高分取较大值，较新存档胜出；使用时间按设备和日期合并。本机设备 ID 不会被导入文件覆盖。
- v22 仍包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和健康统计，云同步也保持关闭。Android 继续支持 v1–v21；旧备份没有桌面小卡片字段时使用默认“今天 / Today”卡片。
...
- Windows 0.2.0 与 Android 旧版共用 v18 结构化 JSON，但不会直接打开或共享 Android Room 数据库。Android 0.5.0 当前导出 v22，Windows 0.2.0 尚不能直接导入；日记和媒体真实文件仍应通过用户选择的普通目录互操作。
- Windows 只保证直接导入 v18。Windows 重新导出会合并 DPAPI 加密影子中的未实现模块与未知字段，但这一机制不是编辑这些字段的界面。
- Windows 0.2.0 没有浏览器、RSS、AI/热量估算、小游戏或健康统计；其手机使用时间页只读取旧 Android v4 文件，不采集 Windows 使用时间，也尚未兼容 Android 0.5.0 的 v22/按设备云同步。
