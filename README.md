...
当前版本：Android **0.9.3**；Windows **0.2.0**。
...
- 手机使用时间：Windows 0.2.0 **只显示、不采集**，但它只认识旧版 Android 手动导出的规范 v4 文件。Android 0.9.3 使用 v27/按设备云同步，因此此 Windows 页面暂不能接收 Android 0.9.3 的新数据；需等待后续 Windows 版本兼容。
...
- Windows 0.2.0 只直接导入/导出 Android v18 JSON。Android 0.9.3 当前导出 v27；v26 已加入笔记库 URI、H1–H6 标题字号与三项主页模块，v27 再加入主页小游戏快捷入口选择。Windows 0.2.0 **不能直接导入 v27**，本次 Android 更新不为旧 Windows 增加兼容层。
...
- “阅读”可从导航/More 页面进入，通过 SAF 导入 TXT 或 PDF；只保存持久读取授权和阅读进度，不复制或改写原书文件。TXT 支持 UTF-8、UTF-16 与 GB18030 回退解码；PDF 在 Android 9+ 优先使用官方 AndroidX PDF 连续增强视图，打开文档或等待首屏超时、服务不可用或加载失败时自动切换到基于 `PdfRenderer` 的连续兼容视图，Android 8 始终使用兼容视图。
...
- Windows 0.2.0 与 Android 旧版共用 v18 结构化 JSON，但不会直接打开或共享 Android Room 数据库。Android 0.9.3 当前导出 v27，Windows 0.2.0 尚不能直接导入；日记和媒体真实文件仍应通过用户选择的普通目录互操作。
- Windows 只保证直接导入 v18。Windows 重新导出会合并 DPAPI 加密影子中的未实现模块与未知字段，但这一机制不是编辑这些字段的界面。
- Windows 0.2.0 没有浏览器、RSS、AI/热量估算、阅读、小游戏或健康统计；其手机使用时间页只读取旧 Android v4 文件，不采集 Windows 使用时间，也尚未兼容 Android 0.9.3 的 v27/按设备云同步。
