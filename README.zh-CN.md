...
当前版本：Android **0.15.0**；Windows **0.7.0**。
...
Android 端以内置 Kotlin Plugin API 作为 Agent 的稳定能力边界：独立 `:plugin-api:core` module 提供插件生命周期、统一的 v2 `PluginContext`，以及日记、Markdown 笔记库、媒体、同步、原生 AI 工具调用、通用 DeskCubby 数据、SAF 受限文件、允许列表内的应用设置/状态、Compose UI contribution 和插件独立存储接口。Agent Runtime 通过这些接口工作，不直接耦合各业务 Screen、ViewModel、Repository 或底层存储；`:app` Adapter 继续执行原有数据边界，非 AI 功能调用链保持不变。架构与扩展步骤见 [android/plugin-api/README.md](android/plugin-api/README.md)。
...
- “导航/More”聚合页可收纳不想直接放在底栏的其他主页面。底栏可见性在“设置 → 底部导航”管理，导航页布局、收纳、模块名称与颜色统一在“设置 → 子页面设置 → 导航页”管理，避免重复开关。聚合页按用户选择的一列/两列/三列分别按真实卡片高度排列；平时卡片不显示手柄，长按任一模块进入布局更改模式后所有卡片出现四点手柄可拖动排序，点右上角对勾退出。每个模块可单独设置名称、按钮底色与模块整体底色。
...
- Markdown 中独占一行的媒体可拖动排序；源码模式的普通文字恢复使用完整编辑宽度，只有独占一行的媒体会在该行右侧局部覆盖删除/拖动按钮。源码和阅读预览中的媒体都提供删除按钮，确认后删除当前日记内该文件的全部引用和媒体文件，并尽力清理对应侧车元数据。删除前仍执行 SHA-256 外部修改检测，日记或媒体文件主操作失败都不会误报成功。
...
- TXT 支持整书搜索、上一项/下一项跳转、高亮结果和长按选择复制。PDFium 增强视图支持双指缩放：手势期间内容围绕捏合中心以矩阵变换实时跟随手指、不重新渲染页面，松手后按最终比例重新渲染当前页；阅读设置可保存 50%–300% 的基准比例。含可提取文字的 PDF 可搜索并自动扫描目录，纯图片扫描件不伪造文字结果。系统兼容视图仍可按真实页码/进度连续阅读，但不提供 PDFium 的文字搜索和目录能力。
...
- 阅读设置可开启“全屏纯净模式”：正文扩展到安全区域，默认隐藏书名、工具栏、页码和系统栏；点屏幕中央唤醒或再次隐藏组件时，正文平面不移动、不缩放，可见控件只覆盖原页面顶部。方向仍可选跟随系统、锁定竖屏或锁定横屏。
...
- 每本书的 URI、封面、页/段进度及全局阅读偏好写入应用私有 schema-v7 JSON，并继续兼容 v1–v6。v7 在首个可见 TXT/PDF 页之外，按 5% 一档向下保存页内位置，因此类似第 13.65 页的位置可在页内恢复，并避免越过尚未阅读的内容；滚动稳定约 600 毫秒后保存，离开阅读器时再补一次检查点。前台阅读时长仍写入独立私有 `engagement-times-v1.json`，其内部 schema v2 会为每本已有时长的书保存最后书名、无损升级 v1 时长，并在书籍移出书架后继续保留书名与时长。页内偏移和这份阅读历史都只留在 Android 私有状态：v31 应用 JSON 与 `reading/v1/progress.json` 仍沿用既有 URI-free 页/段字段，并把对外位置投影到该页开头。完整书籍字节和类型生成 SHA-256 指纹；书名、URI、封面、正文、书架、显示偏好与阅读时长均不随这些进度记录或 Android 系统备份迁移。
...
- 收藏夹页面提供密码保护的私密文本收藏：首次使用设置密码，条目用密码派生密钥（PBKDF2 + AES-GCM）加密后保存在 Room。新密码允许 1 个到任意多个 Unicode 码点；密码不可找回。v31 会原样备份 AES-GCM 密文、IV、盐、迭代次数和加密校验值，但绝不写入密码、明文或派生密钥，因此换设备后输入原密码即可解锁。卡片不显示日期；有备注时直接显示备注，无备注时省略该行。普通正文单击复制，安全 HTTP(S) 链接单击用系统浏览器打开，长按进入含复制/编辑/删除的界面，右侧四点手柄可拖动排序；“设置 → 子页面设置 → 收藏夹”可调整卡片最小行高，最低 48dp，与小巧思的紧凑高度一致。
...
- 小游戏会按各自玩法累计特色统计：2048 同时记录“总操作次数”（每次被游戏接受的方向输入，包含没有移动棋盘的输入）和“有效移动”，另有合并、最高方块及获胜；0.10.0 起不再新增或展示 2048 失败次数/胜率，但旧 Room/备份中的失败字段继续兼容往返。围棋记录落子、提子、停着和完成棋局数，其他游戏继续记录各自的食物、消行、扫雷、移动与胜负。统一“统计”页面汇总日记、手机使用时间、健康、阅读、全部游戏和 Agent 模型用量；Agent 统计展示运行、模型调用、Provider 实际报告的输入/输出/总计/缓存输入/推理 Token、未报告调用及缓存率，不根据文本长度估算缺失用量。阅读排行会在书籍移出书架后继续显示已保存书名，不再改成通用占位名。围棋存档、最高提子、特色统计和主页围棋快捷入口当前只保存在 Android 本机，并从 v31 备份投影排除；既有七个游戏/变体继续按原规则进入 v31。
- 每个小游戏分别累计前台游玩总时长；计时每 30 秒检查点保存，并在离开游戏或应用退到后台时保存到应用私有 `engagement-times-v1.json`。总时长不会进入 Room 游戏存档、v31 JSON 或 Android 系统备份；特色统计存入 Room，既有七个游戏/变体进入 v31 显式备份，围棋统计只留在 Android 本机。
...
- 每次安装会生成不依赖硬件标识的随机稳定设备 ID，并使用可编辑的设备名称。手机使用时间页可选择“所有设备”或任一设备；本机只采集自己的系统数据，其他设备历史来自 v31 导入或云同步。“所有设备”按日期和应用相加，并显示短设备 ID 以区分同名设备。开启后每天首次打开应用也会尝试采集一次。
...
- Room v13 是手机使用时间、其他设备使用时间缓存、健康每日统计、小游戏特色统计和 Agent 历史/Review 元数据的唯一运行时权威来源。升级时会把 `usage-statistics.json`、旧设备缓存和 `step-statistics.json` 事务化、幂等迁入 Room；损坏文件原样保留且不会遮住其他有效文件。JSON 编解码只保留用于旧数据迁移、DeskCubby 备份/导入导出和云同步，外部格式保持兼容。为避免系统备份把同库中的健康明细带到其他设备，`deskcubby.db` 及其 WAL/SHM/journal 均排除 Android 系统备份；显式 DeskCubby JSON/云同步仍是受支持的迁移路径。

### AI 配置与 Agent

- AI 配置库支持多套文字/图片模型；每套配置包含名称、类型、API 地址、模型名称、API Key、温度、系统提示词、HTTP 许可和显式“原生工具调用”能力。旧配置默认关闭该能力；在用户为真正兼容的 Provider/模型启用前，Agent 会明确失败关闭，绝不解析普通自然语言来偷偷执行工具。
- Android AI 现在运行有界的 OpenAI-compatible 原生工具调用循环：用户 → 模型 → 参数校验后的 Tool Call → Tool Result → 模型，直到得到最终回复。一次任务可连续或重复调用多个工具，并处理取消、网络/工具失败、非法调用、重复 ID 和 12 轮上限。
- 输入框左侧的“四个方块”按钮提供“插入图片/文档、管理上下文、AI 权限模式”。单次最多添加 5 个附件，每个上限 8 MiB；同一模型请求实际携带的图片合计另限 8 MiB。图片使用模型多模态能力，PDF、DOCX、TXT、Markdown、HTML、JSON、CSV、XML 只提取有界文本，全部作为不可信数据。
- “管理上下文”会持久保存日记、小巧思、日期、日常事件、笔记、诗词本、手机使用时间和统计数据的独立授权。授权表示 Agent 可以按需检索和读取，不会把整类数据全文塞入 prompt；每轮只先得到允许来源的类型、数量、分类、标题和日期范围等轻量 metadata。旧会话中的冻结上下文继续兼容，但只作为不可信旧数据。
- 可扩展 Tool Registry 首版提供数据列出/搜索/读取/创建/编辑/删除、SAF 授权范围内的文件列出/搜索/读取/创建/修改、受限公网网页搜索/读取，以及允许列表内的应用设置与状态。文件工具不能越出日记或已选笔记根目录；应用工具不能读取或修改 API Key、密码、云凭据、存储授权、Agent 权限等安全敏感设置。
- 只读工具无需审批。“需要批准”模式下，每个 mutation 都会在执行前单独显示工具、目标、计划和可用的 before/after；批准或拒绝会作为 Tool Result 返回模型。“全自动”只跳过弹窗，不跳过每项 Review 记录，也不能把多个操作打包绕过审批。
- 独立 Review 页面按运行/会话展示实际修改的时间、工具、目标、操作、摘要、before/after、状态、详细工具事件和 Undo 可用性。Undo 会真正执行对应的数据、文件或设置恢复，成功后才更新记录；SAF 文件写入继续使用版本冲突检查和恢复安全路径。
- 聊天只显示“正在搜索、读取、等待批准、修改完成”等安全执行摘要，不暴露隐藏 chain-of-thought；工具卡可展开查看有界参数/结果，停止按钮会取消当前运行。Agent 回复按 CommonMark 渲染（标题、列表、引用、代码、链接，可选中文字）。AI 设置改为独立子页：可配置 AI 页面字体大小（12–28 sp）、回复框最大宽度（280–1200 dp）与可一键恢复默认的 Agent 提示词，模型配置移到“AI 配置”子页。工具轮次请求 JSON 已规范序列化（assistant 空内容省略 content 键、tool 消息不带非标准 name 字段），兼容严格 OpenAI 兼容端点。会话、消息、附件、运行、Provider 用量、工具事件与 Review 存入 Room v13，旧聊天通过显式迁移保留。
- 可选的 Agent 聊天云同步使用独立 `agent/v1/chats.json`，通过稳定同步 ID、LWW 和删除 tombstone 合并文字、冻结文档文本、图片占位、已完成运行状态与 Provider 用量；不上传本机 `content://` URI、图片字节、审批/Undo 载荷、秘密或详细 Review 事件。新建云配置默认勾选 Agent 聊天，旧配置保持原选择直到用户编辑。
- 统计只记录 Provider 响应实际返回的 Agent 运行、模型调用、已报告/未报告调用、输入/输出/总计/缓存输入/推理 Token 和缓存率；缺失字段保持未知，不按文本长度估算。

> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v31 JSON/自动备份。v31 还包含 Agent 数据源/权限偏好、自定义主题、Markdown 标题字号、笔记目录引用、既有七个游戏/变体的主页快捷入口、存档与特色统计、桌面小卡片设计及其外观、多设备使用时间、URI-free 阅读进度，以及 Vault 密文/校验元数据；请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。Agent 聊天正文/Review 历史和围棋快捷入口、存档、统计不进入通用 v31 备份。
...
- 设置中可维护多个 WebDAV 或 S3 兼容服务配置，分别启停，并选择日记、媒体、应用 JSON、多设备使用时间、阅读进度、Agent 聊天以及“仅上传”或“双向”同步。新建配置默认包含阅读进度和 Agent 聊天；既有配置不会被升级过程静默勾选，需要用户编辑配置后主动开启。
...
- 选择“应用 JSON”会把 v31 备份以 `json/dc.json` 上传到所选服务，其中包含明文 AI API Key、Agent 数据源/权限偏好、自定义主题、结构化记录和 URI-free 阅读进度。另选“多设备使用时间”会使用每设备独立对象，另选“阅读进度”会使用 `reading/v1/progress.json`，另选“Agent 聊天”会使用 `agent/v1/chats.json`；这些专用对象可独立合并，无需恢复整份应用 JSON。HTTPS 只保护传输过程，远端对象没有端到端加密，请只使用可信服务。
...
- 当前备份格式为 v31，最大 64 MiB，并继续安全导入 v1–v29。它保留 v29 的桌面卡片外观与 URI-free 阅读进度字段，并增加持久化的 `agentEnabledSources`、`agentPermissionMode` 和每个模型配置显式的 `supportsToolCalling` 能力。旧备份默认不授予 Agent 数据源、采用“需要批准”，并关闭原生工具调用。Windows 0.7.0 仍只支持 v1–v29，不能导入 Android v31；在 Windows 后续支持前，请使用 Android 到 Android 的 v31 或各专用内容同步对象。
- Vault 密码/明文/派生密钥、WebDAV 密码、S3 用户名/Key/Session Token、Agent 聊天/附件/运行/Review 历史、笔记/日记正文、媒体文件、背景图片文件、阅读书架/封面/偏好、阅读与小游戏总时长、健康历史和系统权限不进入 JSON。v31 导入后 Vault 保持锁定；既有七个游戏/变体同 ID 最高分取较大值，较新存档胜出，特色统计逐项取较大值；围棋存档、最高提子、特色统计和主页围棋快捷入口从 v31 投影排除，只保存在 Android 本机。使用时间按设备和日期合并，本机设备 ID 不会被导入文件覆盖。
- v31 仍包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和健康统计，云同步也保持关闭；音乐可视化设置会恢复，但录音权限必须由设备本机重新授予。旧备份缺少 Agent、Custom 主题、阅读进度或桌面卡片外观字段时使用安全默认值并保留本机进度；更早版本的逐项兼容规则继续有效。
- 导入 v11 及更早备份时，仅为配置 ID 与 API 地址都一致的 AI 配置保留本机已有 Key。

### 本地数据库

- Room 数据库当前版本为 13，保留 1→2 至 12→13 的全部显式迁移，不使用 destructive migration；10→11 新增手机使用时间、外部设备缓存、健康每日统计和旧文件迁移标记表，11→12 新增小游戏特色统计表；12→13 保留旧 AI 行并增加稳定聊天同步 ID/tombstone、附件、Agent 运行/Provider 用量、工具事件和 mutation Review/Undo 记录。
- v6 新增 AI 会话和消息表；v13 中删除会话改为可同步 tombstone，并释放本机附件 URI 授权。AI 历史不进入通用 JSON，只进入用户单独勾选的 Agent 聊天同步对象。
...
0.15.0（2026-08-14）把 Android AI 设置重构为独立子页：模型配置移到“AI 配置”子页，新增 AI 页面字体大小、回复框最大宽度与可一键恢复默认的 Agent 提示词；Agent 回复渲染 Markdown，工具轮次请求 JSON 规范序列化（不再发送 `content:null`、tool 消息去掉非标准 `name` 字段），修复多轮工具调用在严格 OpenAI 兼容端点上的失败。导航/More 聚合页新增一列/两列/三列布局、长按进入布局更改模式（四点手柄排序、对勾退出）与每模块名称/按钮底色/整体底色。PDF 双指缩放改为矩阵实时跟随，手势结束后再重新渲染。应用 JSON 升至 v31 并继续导入 v1–v30；Android versionCode 升至 31。Windows 保持 0.7.0，源码实现未修改，仍只接受 Android v1–v29 应用 JSON。

0.14.0（2026-08-13）把 Android AI Chat 升级为真实、有界、使用原生工具调用的 Agent Runtime：新增持久化的逐数据源上下文授权、“需要批准/全自动”mutation 策略、基于 Plugin API v2 的可扩展数据/文件/网页/应用工具、可取消的安全执行过程、逐操作审批、独立 Review 与真实 Undo，以及最多五个图片/文档附件。Agent 会话和 Provider 实际报告的用量可通过独立 `agent/v1/chats.json` 同步；统计新增运行、调用、输入/输出/总计/缓存/推理 Token 与缓存率，缺失字段不估算。Room 通过显式迁移升至 v13，应用 JSON 升至 v30 并继续导入 v1–v29，Android versionCode 升至 30。Windows 保持 0.7.0，源码实现未修改，仍只接受 Android v1–v29 应用 JSON。

0.13.2（2026-08-12）让日记 Markdown 源码中的普通文字恢复完整编辑宽度，删除/拖动按钮只在独立媒体行尾部局部覆盖。全屏纯净模式显隐书名、工具栏、页码和系统栏时改为把控件悬浮覆盖在正文上，屏幕中央文字不再上下跳动。私有 engagement JSON 升至内部 schema v2，为每本已有时长的书持久保存最后书名；v1 时长无损升级，书籍移出书架后仍显示书名与历史时长。Android 升至 versionCode 29；Reader 状态保持 schema v7、备份保持 v29、Room 保持 v12；这次 Android 发布时 Windows 仍为 0.6.0。

0.13.1（2026-08-12）把 Android TXT 与 PDF 的恢复位置细化到每页 5%。应用会保存首个可见页和向下取整的 0/5/…/95 页内检查点；滚动稳定约 600 毫秒后写入，离开时再保存一次，因此恢复不会越过未读内容。Reader 私有状态升至 schema v7 并继续读取 v1–v6；更细的页内偏移只留在 Android，本次不改变 v29 与 `reading/v1/progress.json` 的隐私受限页/段结构。Android 升至 versionCode 28；备份保持 v29、Room 保持 v12；这次 Android 发布时 Windows 仍为 0.6.0。
...
同版新增列表/网格书架、书架进度与网格书名显隐、六种页面背景、自定义前景色、衬线/无衬线/等宽字体、自然/两端对齐、520–1280 px 版心和默认专注模式，同时保留字号、行段距、PDF 基准缩放与章节识别设置。Reader 私有状态升至 schema v3 并无损读取 v1/v2；书架路径、书名、正文、偏好和阅读时长仍只留在本机。Windows 升至 0.6.1；SQLite 保持 v7，交换格式保持 v29。
...
- Agent 服务商需要兼容非流式 OpenAI `chat/completions` 的原生 tools/tool_calls 消息格式，并由用户在模型配置中显式启用；不支持时只会提示，绝不从普通回复文本解析并执行工具。聊天图片和图片识别通过 `image_url` data URL 发送，单张图片限制为 8 MiB；界面不展示隐藏 chain-of-thought。
- “管理上下文”只授予按需检索权限，不会直接上传全部数据；具体条目只有被 Agent 工具读取后才会进入当前模型请求。日记、笔记、网页、附件和旧会话冻结内容始终作为不可信数据，不能改变系统提示、授权或审批规则。
...
- 云端应用 JSON 的下载只产生待确认的暂存副本；恢复 JSON 不会替换日记正文、媒体文件或 Agent 对话历史。Agent 聊天仅通过用户在云配置中单独启用的 `agent/v1/chats.json` 合并。
