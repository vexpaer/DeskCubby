...
当前版本：Android **0.14.0**；Windows **0.7.0**。
...
- 每本书的 URI、封面、页/段进度及全局阅读偏好写入应用私有 schema-v7 JSON，并继续兼容 v1–v6。v7 在首个可见 TXT/PDF 页之外，按 5% 一档向下保存页内位置，因此类似第 13.65 页的位置可在页内恢复，并避免越过尚未阅读的内容；滚动稳定约 600 毫秒后保存，离开阅读器时再补一次检查点。前台阅读时长仍写入独立私有 `engagement-times-v1.json`，其内部 schema v2 会为每本已有时长的书保存最后书名、无损升级 v1 时长，并在书籍移出书架后继续保留书名与时长。页内偏移和这份阅读历史都只留在 Android 私有状态：v30 应用 JSON 与 `reading/v1/progress.json` 仍沿用既有 URI-free 页/段字段，并把对外位置投影到该页开头。完整书籍字节和类型生成 SHA-256 指纹；书名、URI、封面、正文、书架、显示偏好与阅读时长均不随这些进度记录或 Android 系统备份迁移。
...
- 收藏夹页面提供密码保护的私密文本收藏：首次使用设置密码，条目用密码派生密钥（PBKDF2 + AES-GCM）加密后保存在 Room。新密码允许 1 个到任意多个 Unicode 码点；密码不可找回。v30 会原样备份 AES-GCM 密文、IV、盐、迭代次数和加密校验值，但绝不写入密码、明文或派生密钥，因此换设备后输入原密码即可解锁。卡片不显示日期；有备注时直接显示备注，无备注时省略该行。普通正文单击复制，安全 HTTP(S) 链接单击用系统浏览器打开，长按进入含复制/编辑/删除的界面，右侧四点手柄可拖动排序；“设置 → 子页面设置 → 收藏夹”可调整卡片最小行高，最低 48dp，与小巧思的紧凑高度一致。
...
- 小游戏会按各自玩法累计特色统计：2048 同时记录“总操作次数”（每次被游戏接受的方向输入，包含没有移动棋盘的输入）和“有效移动”，另有合并、最高方块及获胜；0.10.0 起不再新增或展示 2048 失败次数/胜率，但旧 Room/备份中的失败字段继续兼容往返。围棋记录落子、提子、停着和完成棋局数，其他游戏继续记录各自的食物、消行、扫雷、移动与胜负。统一“统计”页面汇总日记、手机使用时间、健康、阅读、全部游戏和 Agent 模型用量；Agent 统计展示运行、模型调用、Provider 实际报告的输入/输出/总计/缓存输入/推理 Token、未报告调用及缓存率，不根据文本长度估算缺失用量。阅读排行会在书籍移出书架后继续显示已保存书名，不再改成通用占位名。围棋存档、最高提子、特色统计和主页围棋快捷入口当前只保存在 Android 本机，并从 v30 备份投影排除；既有七个游戏/变体继续按原规则进入 v30。
- 每个小游戏分别累计前台游玩总时长；计时每 30 秒检查点保存，并在离开游戏或应用退到后台时保存到应用私有 `engagement-times-v1.json`。总时长不会进入 Room 游戏存档、v30 JSON 或 Android 系统备份；特色统计存入 Room，既有七个游戏/变体进入 v30 显式备份，围棋统计只留在 Android 本机。
...
- 每次安装会生成不依赖硬件标识的随机稳定设备 ID，并使用可编辑的设备名称。手机使用时间页可选择“所有设备”或任一设备；本机只采集自己的系统数据，其他设备历史来自 v30 导入或云同步。“所有设备”按日期和应用相加，并显示短设备 ID 以区分同名设备。开启后每天首次打开应用也会尝试采集一次。
...
- 聊天只显示“正在搜索、读取、等待批准、修改完成”等安全执行摘要，不暴露隐藏 chain-of-thought；工具卡可展开查看有界参数/结果，停止按钮会取消当前运行。会话、消息、附件、运行、Provider 用量、工具事件与 Review 存入 Room v13，旧聊天通过显式迁移保留。
...
> [!WARNING]
> 按当前产品设计，AI API Key 会以**明文**随 AI 配置写入应用设置，也会进入 DeskCubby v30 JSON/自动备份。v30 还包含 Agent 数据源/权限偏好、自定义主题、Markdown 标题字号、笔记目录引用、既有七个游戏/变体的主页快捷入口、存档与特色统计、桌面小卡片设计及其外观、多设备使用时间、URI-free 阅读进度，以及 Vault 密文/校验元数据；请勿把备份文件、应用数据目录、同步到云端的应用 JSON 或含 Key 的截图放入公开或共享位置。Agent 聊天正文/Review 历史和围棋快捷入口、存档、统计不进入通用 v30 备份。
...
- 选择“应用 JSON”会把 v30 备份以 `json/dc.json` 上传到所选服务，其中包含明文 AI API Key、Agent 数据源/权限偏好、自定义主题、结构化记录和 URI-free 阅读进度。另选“多设备使用时间”会使用每设备独立对象，另选“阅读进度”会使用 `reading/v1/progress.json`，另选“Agent 聊天”会使用 `agent/v1/chats.json`；这些专用对象可独立合并，无需恢复整份应用 JSON。HTTPS 只保护传输过程，远端对象没有端到端加密，请只使用可信服务。
...
- 当前备份格式为 v30，最大 64 MiB，并继续安全导入 v1–v29。它保留 v29 的桌面卡片外观与 URI-free 阅读进度字段，并增加持久化的 `agentEnabledSources`、`agentPermissionMode` 和每个模型配置显式的 `supportsToolCalling` 能力。旧备份默认不授予 Agent 数据源、采用“需要批准”，并关闭原生工具调用。Windows 0.7.0 仍只支持 v1–v29，不能导入 Android v30；在 Windows 后续支持前，请使用 Android 到 Android 的 v30 或各专用内容同步对象。
- Vault 密码/明文/派生密钥、WebDAV 密码、S3 用户名/Key/Session Token、Agent 聊天/附件/运行/Review 历史、笔记/日记正文、媒体文件、背景图片文件、阅读书架/封面/偏好、阅读与小游戏总时长、健康历史和系统权限不进入 JSON。v30 导入后 Vault 保持锁定；既有七个游戏/变体同 ID 最高分取较大值，较新存档胜出，特色统计逐项取较大值；围棋存档、最高提子、特色统计和主页围棋快捷入口从 v30 投影排除，只保存在 Android 本机。使用时间按设备和日期合并，本机设备 ID 不会被导入文件覆盖。
- v30 仍包含两个统计功能的普通开关字段，但导入时会强制关闭手机使用时间和健康统计，云同步也保持关闭；音乐可视化设置会恢复，但录音权限必须由设备本机重新授予。旧备份缺少 Agent、Custom 主题、阅读进度或桌面卡片外观字段时使用安全默认值并保留本机进度；更早版本的逐项兼容规则继续有效。
